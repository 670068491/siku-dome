		...
		bindEvent: function(type) {
			...
			function popupDate(e) {
				...
				_self.gearDate.innerHTML = '<div class="date_ctrl slideInUp">' +
					...
					'<div class="date_btn lcalendar_cancel">出生日期</div>' +
			...
			function finishMobileDateTime(e) {
				...
				_self.trigger.value = (date_yy % passY + _self.minY) + "-" + date_mm + "-" + date_dd + " " + (time_hh.length <
					2 ? "0" : "") + time_hh + (time_mm.length < 2 ? ":0" : ":") + time_mm;
				_self.value = _self.trigger.value;
				if (_self.onSubmit) _self.onSubmit();
				closeMobileCalendar(e);
			}
